...
class House3DUtils():
    def __init__(
            ...
            build_graph=True,
            graph_dir='/srv/share/abhshkdz/data/embodied-qa/suncg/3d-graphs',
            target_obj_conn_map_dir='/srv/share/abhshkdz/data/embodied-qa/suncg/target_obj_connmaps',
            debug=True,
    ...
    def step(self, action, step_reward=False):
        ...
        if step_reward == True:
            ...
            x1, y1 = self.env.house.to_grid(self.env.cam.pos.x,
                                            self.env.cam.pos.z)
            init_target_dist = final_target_dist = self.env.house.connMap[x1, y1]
        ...
        if action == 0:
            ...
            if mv == False:  # collision
                ...
                ) / np.abs(self.dirs[self.angles.index(self.env.cam.yaw % 180)]
                           ).sum())
        ...
        # within point-goal success reward
        if final_target_dist <= 5:
    ...
    def build_graph(self, save_path=None):
        ...
        for x in tqdm(range(collide_res + 1)):
                ...
                if self.env.house.canMove(x, y) and pos not in visit:
                    ...
                    while ptr < len(que):
                        ...
                        for deti in range(len(self.dirs)):
                            ...
                            if (self.env.house.inside(tx, ty) and self.
                                    mock_obs_map[min(cx, tx):max(cx, tx) + 1,
                                                 min(cy, ty):
                                                 max(cy, ty) + 1].sum() == 0):
    ...
    def build_graph_from_source(self, save_path=None, target=None):
        assert target != None
        ...
        while ptr < len(que):
            ...
            baseline = np.mean(dists)

            if visit[que[ptr]] >= min(950, self.env.house.connMap.max() - 10):
                break
            ...
            # make forward steps in all directions
            # do proper collision-checking
            # let's see how costly this gets
            for ang_idx, ang in enumerate(self.angles):

                x, y = self.env.house.to_coor(cx, cy, True)
                self.env.reset(x=x, y=y, yaw=ang)

                pre_pos = [
                    self.env.cam.pos.x, self.env.cam.pos.z, self.env.cam.yaw
                ]
                ...
                if all([
                        pre_pos[x] == post_pos[x] for x in range(3)
                ]) == True:  # if same position before/after step, then ignore
                    continue

                tx, ty = self.env.house.to_grid(self.env.cam.pos.x,
                                                self.env.cam.pos.z)
                self.graph.add_edge(
                    (tx, ty, self.angles[(ang_idx + int(len(self.angles) / 2))
                                         % len(self.angles)]),
                    (cx, cy, self.angles[(ang_idx + int(len(self.angles) / 2))
                                         % len(self.angles)]), {
                                             'cost': 1
                                         })
                ...
                tp = (tx, ty)
                if len(dists) > 10:
                    if tp not in visit and self.env.house.connMap[tx,
                                                                  ty] > baseline:
                        visit[tp] = self.env.house.connMap[tx, ty]
                        dists.append(self.env.house.connMap[tx, ty])
                        que.append(tp)
                else:
                    if tp not in visit:
                        visit[tp] = self.env.house.connMap[tx, ty]
    ...
    def build_graph_from_source_approximate_collision(self,
                                                      save_path=None,
                                                      target=None):
        assert target != None
        ...
        while ptr < len(que):
            ...
            print(len(que[ptr:]), visit[que[ptr]])
            ...
            # [TODO] experiment with 3rd parameter as True / False
            x, y = self.env.house.to_coor(cx, cy)
            self.env.reset(x=x, y=y, yaw=0)
            ...
            for ang_idx, ang in enumerate(self.angles):
                ...
                # update angle
                self.env.cam.yaw = ang
                self.env.cam.updateDirection()

                # move a step
                post_pos = pos + self.env.cam.front * self.move_sensitivity
                tx, ty = self.env.house.to_grid(post_pos.x, post_pos.z)
                if tx < 0 or ty < 0:
                    continue

                # did we collide?
                # print(cx, cy, tx, ty)
                if self.env._check_collision_really_fast(cx, cy, tx, ty) == False:
                    continue

                # all good, add an edge
                self.graph.add_edge(
                    (tx, ty, self.angles[(ang_idx + int(len(self.angles) / 2)) % len(self.angles)]),
                    (cx, cy, self.angles[(ang_idx + int(len(self.angles) / 2)) % len(self.angles)]),
                    {'cost': 1})

                tp = (tx, ty)
                if tp not in visit:
                    visit[tp] = self.env.house.connMap[tx, ty]
                    if self.env.house.connMap[tx, ty] < 0:
    ...
    def render_video_from_pos_queue(self,
        ...
        for fp in fps:
            subprocess.Popen([
                '/usr/local/bin/ffmpeg', '-f', 'image2', '-r',
                str(fp), '-i',
    ...
    def set_target_object(self, obj, room):
        if isinstance(room, str):
            object_tp = room + '_' + obj['id'] + '_' + obj['fine_class'].lower()
        else:
